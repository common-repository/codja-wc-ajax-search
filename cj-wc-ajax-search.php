	...
	if ( in_array( 'woocommerce/woocommerce.php', apply_filters( 'active_plugins', get_option( 'active_plugins' ) ) ) ) {
		...
		class Cj_WooCommerce_Ajax_Search {
			...
			private function __construct() {
				...
				load_plugin_textdomain( 'cj-wc-ajax-search', false, basename( dirname( __FILE__ ) )  .'/languages' );
			...
			private function enqueueScripts() {
				if ( is_rtl() ) {
					wp_enqueue_style( 'cj-wc-ajax-search-style', CJ_WC_AJAX_SEARCH_URL . 'public/css/style-rtl.css');
				} else {
					wp_enqueue_style( 'cj-wc-ajax-search-style', CJ_WC_AJAX_SEARCH_URL . 'public/css/style.css');
				}

				wp_add_inline_style( 'cj-wc-ajax-search-style', stripslashes(htmlspecialchars_decode($this->settings['custom_css'])) );

				wp_enqueue_script( 'cj-wc-ajax-search-script', CJ_WC_AJAX_SEARCH_URL . 'public/js/script.js', array('jquery'), null, true );
			...
			public function adminEnqueueScript( $hook_suffix ) {
				...
				if ( $hook_suffix == 'settings_page_cj-wc-ajax-search-settings' ) {
					wp_enqueue_style( 'cj-wc-ajax-search-style', CJ_WC_AJAX_SEARCH_URL . 'public/css/admin/style.css');
					wp_enqueue_script( 'cj-wc-ajax-search-script', CJ_WC_AJAX_SEARCH_URL . 'public/js/admin/admin-script.js', array('jquery'), null, true );
				}
			}
			...
			public function adminMenu() {
				...
				add_options_page(
					__('WooCommerce Ajax Search', 'cj-wc-ajax-search'),
					__('WooCommerce Ajax Search', 'cj-wc-ajax-search'),
					'manage_options',
			...
			public static function activation() {
				...
				$defaultSettings = array(
					...
					'str_length' => 3,
					'product_count' => 5,
					'loader_image' => CJ_WC_AJAX_SEARCH_URL . 'public/images/ajax-loader.gif',
